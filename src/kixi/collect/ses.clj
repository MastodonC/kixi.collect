(ns kixi.collect.ses
  (:require [amazonica.aws.simpleemail :as email]
            ...
            [kixi.comms :as c]
            [kixi.collect :as m]
            [kixi.collect.destination :as md]
            [kixi.collect.message :as mm]
            [kixi.collect.heimdall :as h]
...
(defmethod c/command-payload
  [:kixi.collect/send-group-mail "1.0.0"]
  [_]
...
(defmethod c/event-payload
  [:kixi.collect/group-mail-accepted "1.0.0"]
  [_]
  (s/keys :req [::m/destination]
          :opt [::m/source]))

(defmethod c/event-payload
  [:kixi.collect/group-mail-rejected "1.0.0"]
  [_]
  (s/keys :req [::m/destination
                ::m/reason]
          :opt [::m/source
...
(defn invalid-rejected
  [cmd explaination]
  {:kixi.comms.event/key :kixi.collect/mail-rejected
...
(defn accepted
  [cmd ses-resp]
  {:kixi.comms.event/key :kixi.collect/mail-accepted
   :kixi.comms.event/version "1.0.0"
...
(defn group-accepted
  ...
  [(merge {:kixi.event/type :kixi.collect/group-mail-accepted
...
(defn group-rejected
  ...
  ([{:keys [::m/destination ::m/source] :as cmd} reason message]
   [(merge {:kixi.event/type :kixi.collect/group-mail-rejected
            :kixi.event/version "1.0.0"
            :kixi.collect.reject/reason reason
            :kixi.collect/destination destination}
           (when source
             {:kixi.collect/source source})
           (when message
             {:kixi.collect.reject/message message}))
    {:partition-key (get-in cmd [:kixi/user :kixi.user/id])}]))
...
(defrecord Mailer
  ...
  component/Lifecycle
  (start [component]
    (log/info "Starting SES collect" directory)
    (merge component
           (when-not send-mail-handler
             {:send-mail-handler
              (c/attach-command-handler!
               communications
               :kixi.collect/collect
           ...
           (when-not send-group-mail-handler
             {:send-group-mail-handler
              (c/attach-validating-command-handler!
               communications
               :kixi.collect/collect-group
               :kixi.collect/send-group-mail
               "1.0.0" (create-group-mail-sender directory endpoint base-url))})))
  (stop [component]
    (log/info "Stopping SES collect")
    (-> component
