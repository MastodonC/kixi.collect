  ...
  (let [uid (uuid)
    ...
    (when-success dr
      ...
      (let [event (wait-for-events uid :kixi.collect/collection-request-rejected)]
        (is (= :invalid-cmd (:kixi.collect.request.reject/reason event))))
      #_(stest/instrument))))
  ...
  (let [uid (uuid)
    ...
    (when-success dr
      ...
      (let [event (wait-for-events uid2 :kixi.collect/collection-request-rejected)]
        (is (= :unauthorised (:kixi.collect.request.reject/reason event)))))))
...
(deftest collect-request-incorrect-type
  (let [uid (uuid)
    ...
    (when-success dr
      (send-request-cmd uid message groups (get-in dr [:body ::ms/id]))
      (let [event (wait-for-events uid :kixi.collect/collection-request-rejected)]
        (is (= :incorrect-type (:kixi.collect.request.reject/reason event)))))))
